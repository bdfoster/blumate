...
from blumate.const import PLATFORM_FORMAT
from blumate.util import OrderedSet
...
def prepare(hass):
    ...
    import blumate.components as components

    AVAILABLE_COMPONENTS.clear()

    AVAILABLE_COMPONENTS.extend(
        item[1] for item in
        pkgutil.iter_modules(components.__path__, 'blumate.components.'))
...
def get_component(comp_name):
    ...
    potential_paths = ['custom_components.{}'.format(comp_name),
                       'blumate.components.{}'.format(comp_name)]
